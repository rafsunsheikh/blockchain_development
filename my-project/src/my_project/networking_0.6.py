# Set the loop to keep connection open for the clients
...
class ConnectionPool:
    ...
    def broadcast(self, writer, message):
        for user in self.connection_pool:
            if user != writer:
                # we dont need to broadcast to the user sending the message also
...
async def handle_connection(reader, writer):
    ...
    # We're now outside the message loop, and usr has quit. Let's close the connection and clean up
    writer.close()
